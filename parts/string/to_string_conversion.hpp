...
template <typename T>
struct SConversionType
    : public ConversionTypeSelector<T, std::is_integral<T>::value && std::is_signed<T>::value> {};
...
[[nodiscard]] constexpr auto IntToString(IT_T begin, IT_T end, T value, uint_fast8_t base)
    -> size_t {
...
template <bool CHECK_OVERFLOW, typename T, typename IT_T = char*>
[[nodiscard]] constexpr auto FloatToString(IT_T begin, IT_T end, T value, uint_fast8_t base)
    -> size_t {
    if constexpr (CHECK_OVERFLOW) {
        if (begin == end) {
            return 1;
...
[[nodiscard]] constexpr auto ToString(IT_T begin, IT_T end, const T& value, uint_fast8_t base)
    -> size_t {
