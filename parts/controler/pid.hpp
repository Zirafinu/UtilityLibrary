...
class CPidControl {
   public:
    ...
    explicit CPidControl(
        const SParameterSet& params = {},
        const SLimitSet& limits = SLimitSet{std::numeric_limits<OUTPUT_TYPE>::lowest(),
    ...
    void SetLimits(const SLimitSet& limits) noexcept { this->m_limits = limits; }
    void SetSetPoint(const OUTPUT_TYPE& value) noexcept { m_setpoint = value; }

    [[nodiscard]] auto GetParameters() const noexcept -> const SParameterSet& {
        return m_parameters;
    }
    [[nodiscard]] auto GetLimits() const noexcept -> const SLimitSet& { return m_limits; }
    [[nodiscard]] auto GetSetPoint() const noexcept -> const OUTPUT_TYPE& { return m_setpoint; }
