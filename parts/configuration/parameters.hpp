...
public:
	...
	[[nodiscard]] virtual auto ConvertValueToString(
			OutputStringBuffer string) const -> UsedCharacters = 0;

	[[nodiscard]] virtual auto ParseValueFromString(const ValueString &string) ->
			ErrorMessage = 0;
...
public:
	...
	[[nodiscard]] static auto Value() noexcept -> const TYPE & {
		return GetInstance().m_value;
	}

	[[nodiscard]] auto ConvertValueToString(
			OutputStringBuffer string) const -> UsedCharacters override {
		return m_converter.Stringify(string, m_value);
	}

	[[nodiscard]] auto ParseValueFromString(const ValueString &string) ->
			ErrorMessage override {
          return m_converter.Parse(string, NotNull<TYPE>(&m_value));
	}

	void ResetValue() override {
		m_value = mc_default_value;
	}

	[[nodiscard]] auto GetValue() const -> const TYPE & {
		return m_value;
	}

	[[nodiscard]] auto GetValue() -> TYPE & {
		return m_value;
	}
...
protected:
	explicit CPlainParameter(const char *name, const char *description,
			const TYPE &default_value,
			const CAValueConverter<TYPE>& pc_converter) noexcept :
			CAParameter(name, description, ID), m_value(default_value), mc_default_value(
					default_value), m_converter { pc_converter } {
	}
};
