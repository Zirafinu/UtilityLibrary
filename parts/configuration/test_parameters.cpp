...
TEST_SUITE("parameters") {
	TEST_CASE("initial serial") {
		...
		std::inner_product(tmp.begin(), tmp.end(),	cmp.begin(), 0, std::plus<>(), [](uint8_t a, uint8_t b){
			CHECK(a == b);
			return 1;
		});
	}
	...
	TEST_CASE("serial 1-8") {
		...
		CHECK(ToString(SerialParam::GetInstance().ParseValueFromString(txt)) == ToString(SerialParam::NO_ERROR));
