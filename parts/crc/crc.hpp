...
template<typename T = uint16_t, T POLYNOM = 0x8005, // NOLINT(readability-magic-numbers,cppcoreguidelines-avoid-magic-numbers)
		std::size_t BITS_IN_TABLE = CHAR_BIT, bool REFLECT_DATA = false> // NOLINT(readability-magic-numbers,cppcoreguidelines-avoid-magic-numbers)
class CCrcBase {
	static_assert(!std::is_signed<T>::value);
	static_assert(std::is_integral<T>::value);
	static_assert(BITS_IN_TABLE == CHAR_BIT || BITS_IN_TABLE == CHAR_BIT / 2 || BITS_IN_TABLE == 2); // NOLINT(readability-magic-numbers,cppcoreguidelines-avoid-magic-numbers)
	...
	constexpr inline static const auto TABLE_BITS_MASK = (1U << BITS_IN_TABLE)
			- 1U;
...
public:
	...
	[[nodiscard]] constexpr static auto AppendByte(uint8_t value,
			ValueType crc_value) noexcept -> ValueType {
		...
		for (int8_t i = CHAR_BIT - BITS_IN_TABLE; i >= 0; i -= BITS_IN_TABLE) {
			...
			crc_value ^= TABLE[unsigned(selected_bits ^ value_bits)
					& TABLE_BITS_MASK];
		}
		return crc_value;
	}
	...
	void AddData(U value) noexcept {
		...
		for (std::size_t i = sizeof(U); i > 0; --i) {
			m_crc_value = m_ccrc_base.AppendByte(
					uint8_t(value >> unsigned((i - 1) * CHAR_BIT)), m_crc_value);
		}
	}

	void AddData(const uint8_t *start, const uint8_t *end) noexcept {
		while (start != end) {
			m_crc_value = m_ccrc_base.AppendByte(*start++, m_crc_value); //NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
		}
	}
